...
public class ProductVariantController {
    ...
    public ResponseEntity<ProductVariant> CreateProductVariant(
            @Valid @RequestBody ProductVariant productVariant, @PathVariable Long id) throws AppException {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(this.productVariantService.createProductVariant(id, productVariant));
    }

    @GetMapping
    @ApiMessage("Get Product Variant success")
    public ResponseEntity<ProductVariantResponse> GetProductVariantByProductIdAndColorAndSize(
            @RequestParam Long productId, @RequestParam String color, @RequestParam String sizep) throws AppException {
    ...
    public ResponseEntity<List<ProductVariantResponse>> GetAllProductVariant(@PathVariable Long id)
            throws AppException {
    ...
    public ResponseEntity<ProductVariantResponse> UpdateProductVariant(
            @Valid @PathVariable Long id, @RequestBody ProductVariantRequest productVariant) throws AppException {
