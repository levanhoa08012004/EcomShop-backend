...
public class OrderController {
    ...
    @PostMapping("/cash")
    public ResponseEntity<OrderResponseDetail> createOrderByCash(@Valid @RequestBody OrderRequestByCash orderRequest)
            throws AppException {
    ...
    @GetMapping
    public ResponseEntity<ResultPaginationDTO> getOrders(
            @RequestParam(required = false) String status, Pageable pageable) {
        return ResponseEntity.ok(this.orderService.getAllOrder(status, pageable));
    }

    @PutMapping("/status-admin")
    public ResponseEntity<OrderResponseDetail> changeStatusOrder(
            @RequestParam Long orderId, @RequestParam StatusOrder status) throws AppException {
        return ResponseEntity.ok().body(this.orderService.changeStatusOrder(orderId, status));
    }

    @PutMapping("/status")
    public ResponseEntity<OrderResponseDetail> changeStatusOrderShipperAndUser(
            @RequestParam Long orderId, @RequestParam StatusOrder status) throws AppException {
