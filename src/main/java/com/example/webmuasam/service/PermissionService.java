...
public class PermissionService {
    ...
    public Permission updatePermission(Permission permission) throws AppException {
        Permission oldPermission = this.permissionRepository
                ...
                .orElseThrow(() -> new AppException("Permission không tồn tại"));
        if (this.permissionRepository.existsByApiPathAndMethodAndModuleAndIdNot(
                permission.getApiPath(), permission.getMethod(), permission.getModule(), permission.getId())) {
            throw new AppException("Permission đã tồn tại");
        }
    ...
    public void deletePermission(long id) throws AppException {
        Permission permission =
                this.permissionRepository.findById(id).orElseThrow(() -> new AppException("Permission không tồn tại"));
