...
public class ReviewService {
    ...
    public ReviewService(
            ReviewRepository reviewRepository, ProductRepository productRepository, UserRepository userRepository) {
    ...
    public ReviewResponse addReview(
            ...
            Integer rating,
            Instant createdAt,
            List<MultipartFile> imageBases,
            Long productId,
            Long userId)
            throws AppException {
        Product product = this.productRepository
                .findById(productId)
                .orElseThrow(() -> new AppException("Không tìm thấy sản phẩm này"));
    ...
    public ReviewResponse convertReview(Review review) {
        ...
        if (review.getUser().getImage() != null) {
            userReview.setImage(
                    Base64.getEncoder().encodeToString(review.getUser().getImage()));
        }
    ...
    public ResultPaginationDTO getAllReviewByProduct(Pageable pageable, Long productId) {
        ...
        List<ReviewResponse> reviewList =
                reviews.getContent().stream().map((this::convertReview)).collect(Collectors.toList());
